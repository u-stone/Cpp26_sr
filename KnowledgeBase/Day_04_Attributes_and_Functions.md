...
- **`std::meta::members_of(info)`**: Unlike `nonstatic_data_members_of`, this function returns *all* members of a class, including member functions (static and non-static), nested types, and static data members.
...
- **Universal Member Inspection**:
    - `constexpr auto all_members = std::meta::members_of(^MyService);` catches everything.
    - Filtering via `is_function(m)` and `is_nonstatic_data_member(m)` allows categorizing members.
- **Attribute-Driven Logic**:
    - `std::meta::has_attribute(m, ^deprecated)` allows the compiler (and the developer) to detect if a function should be ignored or treated specially (e.g., in a generated documentation tool).
